    ...
    <div>
      ...
      <div className="mb-3">
        <CieoSelect
          ...
          disabled={isDisabled}
          placeholder="-- Grupo histológico --"
          options={grupoKeys.map((key) => ({ value: key, label: key }))}
          ariaLabel="Seleccionar grupo histológico"
        />
      </div>
      ...
      <div className="mb-3">
        <CieoSelect
          key={grupo}
          onValueChange={handleHistChange}
          disabled={isDisabled || !grupo}
          placeholder="-- Tipo histológico --"
          options={currentHistos.map((h) => ({
            value: `${h.c}|${h.d}`,
            label: `${h.c} — ${h.d}`,
          }))}
          ariaLabel="Seleccionar tipo histológico (morfología)"
        />
      </div>
